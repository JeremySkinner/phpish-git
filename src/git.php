...
class Git {
    public function __construct($dir, $settings) {
        ...
        $this->log = new Logger($settings->debug);
    }
    
    public static function createFromWorkingDirectory($settings) {
        return new Git(self::getGitDirectory(), $settings);
    }

    public static function getGitDirectory() {
        $rtn;
        $out=[];
    ...
    public function inDotGitOrBareRepoDir() {
        if (strpos(getcwd(), $this->gitDir) === 0) {
            return true;
        }
        return false;
    }

    function getGitStatus($force = false) {
        ...
        if ($enabled && $gitDir) {
            ...
            if($settings->enableFileStatus && !$this->inDotGitOrBareRepoDir($gitDir)) {
                ...
                if(!$branch) { 
                    $branch = $this->getGitBranch($gitDir, $log); 
                }
